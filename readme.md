...
**Цель:** дообучить модель ruT5-large для решения задач генерации текста на русском языке. 
...
1. ```
    ...
    top_p=0.95
   ```
`input_text` = Придумайте название для книги на основе следующего описания сюжета. Молодой парень находит портал в параллельный мир, где его жизнь намного хуже, чем она была до этого.
...
- **BLEU**: `0.0634` - низкое значение метрики точности указывает на ограниченное совпадение между сгенерированными и эталонными текстами
- **ROUGE-1**: `0.0915` - метрика полноты демонстрирует невысокий уровень перекрытия униграмм между предсказаниями и эталонами
- **METEOR**: `0.1972` - наилучший результат среди всех метрик, что свидетельствует о более высоком качестве генераций, благодаря учету синонимов и морфологических вариаций
