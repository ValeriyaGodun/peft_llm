...
В данном проекте применяются два метода Parameter-Efficient Fine-Tuning (PEFT): Low-Rank Adaptation (LoRA) и Prompt Tuning. Также используется ClearML для отслеживания деталей эксперимента.
...
В проекте используется ClearML для отслеживания экспериментов:
- Логирование гиперпараметров
- Сохранение лучшей модели
- Отслеживание метрик обучения и валидации
...
Для генерации текста используется функция `generate_predictions`, которая:
...
```
# Результат
"Портал в параллельный мир" - это роман, в котором молодой человек обнаруживает портал в параллельный мир, где его жизнь намного хуже, чем была до этого.
```
...
В дополнение к этому можно рассмотреть возможность дополнения автоматических метрик человеческой оценкой для более точной оценки корректности генераций. Однако данный подход требует значительных временных и финансовых ресурсов для привлечения разметчиков и организации всего процесса разметки, что делает его менее доступным по сравнению с автоматическими метриками.